...
void send_set_controlled_entity(ENetPeer *peer, uint16_t eid, uint32_t time) {
...
void deserialize_set_controlled_entity(ENetPacket *packet, uint16_t &eid, uint32_t &time) {
    uint8_t *ptr = packet->data;
    ptr += sizeof(uint8_t);
    eid = *(uint16_t *)(ptr);
    ptr += sizeof(uint16_t);
